...
def compare_images():
    ...
    f1 = os.path.join(opt.testdir,'im_compare_metrics_%s.csv' % (t_now))
    f2 = os.path.join(opt.testdir,'im_compare_names_%s.csv' % (t_now))
    f3 = os.path.join(opt.testdir,'im_compare_metrics_average_%s.csv' % (t_now))
    ...
    images = glob.glob(opt.testdir + '/*real_B%s' % (ext))
